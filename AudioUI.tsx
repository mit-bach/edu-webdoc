  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      for (let chIdx = 0; chIdx < CHAPTERS.length; chIdx++) {
        ...
        const chSections = extractSectionsFromChapter(
          currentArticle,
          CHAPTERS[chIdx].num,
          globalOffset,
        );
      ...
      for (let i = 0; i < allSections.length; i++) {
        ...
        try {
          ...
          if (msg.includes("429")) {
            setProgress((prev) => ({
              ...prev,
              label: "Rate limited, waiting 30s...",
            }));
          ...
          if (msg.includes("OPENAI_API_KEY not set")) {
            setError(
              "Server's OPENAI_API_KEY is not configured. Add it to your .env file and restart the server.",
            );
            break;
          }
        }
    ...
    >
      ...
      >
        ...
        <div className="p-5 space-y-4">
          ...
          >
            {localDev ? (
              <Server size={14} style={{ color: "var(--sage-bright)" }} />
            ) : (
            ...
            <span
              style={{
                fontFamily: "var(--font-body)",
                fontSize: "0.75rem",
                color: localDev ? "var(--sage-bright)" : "var(--text-muted)",
              }}
            >
              {localDev
          ...
          {!localDev && (
            <div>
              ...
              >
                Your key stays in your browser. It is never stored or sent
            ...
            <div className="grid grid-cols-3 gap-2">
              {VOICES.map((v) => (
                <button
                  ...
                  style={{
                    background:
                      voice === v.id ? "var(--amber-glow)" : "var(--bg-deep)",
          ...
          {showAdvanced && (
            ...
            >
              ...
              >
                ...
                ~24,000 words x ~$0.015/1K chars ~ $1.50-3.00 total. Uses{" "}
                <code className="code-inline">gpt-4o-mini-tts-2025-12-15</code>{" "}
                model.
                {localDev
...
export function SectionPlayButton({ sectionId }: { sectionId: string }) {
  const { availableSections, currentSectionId, isPlaying, togglePlayPause } =
    useAudio();
...
export function AudioPlayerBar() {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 md:px-6">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            style={{
              color:
                currentIdx > 0 ? "var(--text-secondary)" : "var(--text-muted)",
          ...
          <button
            onClick={() =>
              isPlaying ? pause() : currentSectionId ? resume() : null
            }
            ...
            style={{
              color:
                currentIdx < sections.length - 1
                  ? "var(--text-secondary)"
                  : "var(--text-muted)",
              opacity: currentIdx < sections.length - 1 ? 1 : 0.3,
            }}
          >
            <SkipForward size={16} />
          </button>
          ...
          >
            {rates.map((rate) => (
              <button
                ...
                style={{
                  background:
                    playbackRate === rate ? "var(--amber-glow)" : "transparent",
                  color:
                    playbackRate === rate
                      ? "var(--amber-bright)"
                      : "var(--text-muted)",
      ...
      {expanded && (
        <div
          className="md:hidden flex items-center justify-between px-4 pb-3 pt-1"
        ...
        >
          ...
          <button
            ...
            style={{
              background: highlightingEnabled
                ? "var(--amber-glow)"
                : "var(--bg-deep)",
...
export function useInjectSectionPlayButtons(
  ...
  useEffect(() => {
    ...
    container
      .querySelectorAll(".injected-play-btn")
